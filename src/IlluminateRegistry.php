...
class IlluminateRegistry implements ManagerRegistry {
	 ...
	 * @const
	 */
	const MANAGER_BINDING_PREFIX = 'odm.managers.';

	/**
	 * @const
	 */
	const CONNECTION_BINDING_PREFIX = 'odm.connections.';
	 ...
	 */
	public function addManager($manager, Config $config)
	{
		$this->container->singleton($this->getManagerBindingName($manager), function () use ($config) {
			return $this->factory->create($config);
		});

		$this->managers[$manager] = $manager;

		$this->addConnection($manager, $config);
	}
	...
	public function getAliasNamespace($alias)
	{
		...
		throw MongoDBException::unknownEntityNamespace($alias);
	}
